...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    ...
    public sealed class JobDispatcher : AgentService, IJobDispatcher
    {
        ...
        public void Run(AgentJobRequestMessage jobRequestMessage)
        {
            ...
            newDispatch.WorkerDispatch = RunAsync(jobRequestMessage, currentDispatch, newDispatch.WorkerCancellationTokenSource.Token, newDispatch.WorkerExitCancellationTokenSource.Token);
        ...
        private async Task RunAsync(AgentJobRequestMessage message, WorkerDispatcher previousJobDispatch, CancellationToken jobRequestCancellationToken, CancellationToken workerExitTimeoutToken)
        {
            ...
            using (var workerProcessCancelTokenSource = new CancellationTokenSource())
            {
                ...
                using (var processInvoker = HostContext.CreateService<IProcessInvoker>())
                {
                    ...
                    try
                    {
                        ...
                        completedTask = await Task.WhenAny(workerProcessTask, Task.Delay(-1, workerExitTimeoutToken));
        ...
        private class WorkerDispatcher : IDisposable
        {
            ...
            public CancellationTokenSource WorkerExitCancellationTokenSource { get; private set; }
            ...
            public WorkerDispatcher(Guid jobId, long requestId)
            {
                JobId = jobId;
                RequestId = requestId;
                WorkerExitCancellationTokenSource = new CancellationTokenSource();
                WorkerCancellationTokenSource = new CancellationTokenSource();
            }

            public bool Cancel(TimeSpan timeout)
            {
                if (WorkerCancellationTokenSource != null && WorkerExitCancellationTokenSource != null)
                {
                    lock (_lock)
                    {
                        if (WorkerCancellationTokenSource != null && WorkerExitCancellationTokenSource != null)
                        {
                            ...
                            WorkerExitCancellationTokenSource.CancelAfter(timeout.Subtract(TimeSpan.FromSeconds(15)));
                            return true;
                        }
                    }
                }
                ...
                if (disposing)
                {
                    if (WorkerCancellationTokenSource != null || WorkerExitCancellationTokenSource != null)
                    {
                        lock (_lock)
                        {
                            ...
                            if (WorkerExitCancellationTokenSource != null)
                            {
                                WorkerExitCancellationTokenSource.Dispose();
                                WorkerExitCancellationTokenSource = null;
                            }
                        }
                    }
                }
